...
ModelDataPathPrefix = Path("/mnt/data/zhengkjiang/pretrained_weights")
...
class Wholebody:
    ...
    def get_kpts24(self, oriImg, single_person=True):
        det_result = inference_detector(self.session_det, oriImg)
        if single_person and len(det_result)>1:
            max_idx = np.argmax(det_result[:,3] - det_result[:,1])
            det_result = det_result[max_idx:max_idx + 1]
        ...
        new_keypoints_info = np.insert(
            keypoints_info, 23, neck, axis=1)

        mmpose_idx = [
            0, 23, 6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15, 2, 1, 4, 3, 22, 21, 20, 19, 18, 17
        ]
        openpose_idx = [

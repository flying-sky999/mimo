...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        background_image: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        ...
        background_ref_image_tensor = self.image_processor.preprocess(
            background_image, height=height, width=width
        )  # (bs, c, width, height) 
        ref_image_tensor = ref_image_tensor.to(
            dtype=self.vae.dtype, device=self.vae.device
        )
        background_ref_image_tensor = background_ref_image_tensor.to(   
            dtype=self.vae.dtype, device=self.vae.device
        )
        ref_image_latents = self.vae.encode(ref_image_tensor).latent_dist.mean * 0.18215
        background_ref_image_latents = self.vae.encode(background_ref_image_tensor).latent_dist.mean * 0.18215
        ...
        print('num_frames', num_frames, 'tile_size', tile_size, 'tile_overlap', tile_overlap)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    # ref_image_latents = torch.cat([ref_image_latents, background_ref_image_latents], dim=2)
                    ref_image_latents = background_ref_image_latents
                    self.reference_net(
